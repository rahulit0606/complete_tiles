...
import { Plus, Edit, Trash2, Upload, Save, BarChart3, Store } from 'lucide-react';
import { Tile } from '../types';
import { useAppStore } from '../stores/appStore';
import { BulkUpload } from './BulkUpload';
import { AnalyticsDashboard } from './AnalyticsDashboard';
import { QRCodeManager } from './QRCodeManager';
import { uploadTile, updateTile, deleteTile, getSellerProfile } from '../lib/supabase';

export const SellerDashboard: React.FC = () => {
  const { currentShowroom, currentUser } = useAppStore();
  ...
  const handleAddTile = async () => {
    try {
      if (!currentShowroom || !currentUser) return;
  ...
  // Filter tiles to show only seller's tiles
  const sellerTiles = currentShowroom?.tiles.filter(tile => 
    tile.showroomId === currentShowroom.id
  ) || [];
      ...
      {activeTab === 'tiles' && (
        <>
          <div className="flex justify-between items-center mb-4">
            <p className="text-gray-600">Manage your tile inventory</p>
            <button
